// Code generated by goctl. DO NOT EDIT.

package user

import (
	"context"
	"database/sql"
	"fmt"
	"strings"
	"time"
	"zerocms/api/model"

	"github.com/zeromicro/go-zero/core/stores/builder"
	"github.com/zeromicro/go-zero/core/stores/cache"
	"github.com/zeromicro/go-zero/core/stores/sqlc"
	"github.com/zeromicro/go-zero/core/stores/sqlx"
	"github.com/zeromicro/go-zero/core/stringx"
)

var (
	sysUserFieldNames          = builder.RawFieldNames(&SysUser{})
	sysUserRows                = strings.Join(sysUserFieldNames, ",")
	sysUserRowsExpectAutoSet   = strings.Join(stringx.Remove(sysUserFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	sysUserRowsWithPlaceHolder = strings.Join(stringx.Remove(sysUserFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"

	cacheSysUserIdPrefix        = "cache:sysUser:id:"
	cacheSysUserEmailPrefix     = "cache:sysUser:email:"
	cacheSysUserLoginNamePrefix = "cache:sysUser:loginName:"
	cacheSysUserPhonePrefix     = "cache:sysUser:phone:"
)

type (
	sysUserModel interface {
		Insert(ctx context.Context, data *SysUser) (sql.Result, error)
		FindOne(ctx context.Context, id int64) (*SysUser, error)
		FindOneByEmail(ctx context.Context, email sql.NullString) (*SysUser, error)
		FindOneByLoginName(ctx context.Context, loginName sql.NullString) (*SysUser, error)
		FindOneByPhone(ctx context.Context, phone sql.NullString) (*SysUser, error)
		Update(ctx context.Context, data *SysUser) error
		Delete(ctx context.Context, id int64) error
	}

	defaultSysUserModel struct {
		sqlc.CachedConn
		table string
	}

	SysUser struct {
		Id        int64          `db:"id"`
		LoginName sql.NullString `db:"login_name"`
		Email     sql.NullString `db:"email"`
		Phone     sql.NullString `db:"phone"`
		Password  sql.NullString `db:"password"`
		Salt      sql.NullString `db:"salt"`
		Nickname  sql.NullString `db:"nickname"`
		Realname  sql.NullString `db:"realname"`
		Gender    int64          `db:"gender"`
		Birthday  sql.NullTime   `db:"birthday"`
		UserType  int64          `db:"user_type"`
		Avatar    sql.NullString `db:"avatar"`
		LoginIp   sql.NullString `db:"login_ip"`
		LoginAt   sql.NullInt64  `db:"login_at"`
		Status    int64          `db:"status"`
		CreatedAt time.Time      `db:"created_at"`
		UpdatedAt time.Time      `db:"updated_at"`
		DeletedAt sql.NullTime   `db:"deleted_at"`
	}
)

func newSysUserModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultSysUserModel {
	return &defaultSysUserModel{
		CachedConn: sqlc.NewConn(conn, c, opts...),
		table:      "`sys_user`",
	}
}

func (m *defaultSysUserModel) withSession(session sqlx.Session) *defaultSysUserModel {
	return &defaultSysUserModel{
		CachedConn: m.CachedConn.WithSession(session),
		table:      "`sys_user`",
	}
}

func (m *defaultSysUserModel) Delete(ctx context.Context, id int64) error {
	data, err := m.FindOne(ctx, id)
	if err != nil {
		return err
	}

	sysUserEmailKey := fmt.Sprintf("%s%v", cacheSysUserEmailPrefix, data.Email)
	sysUserIdKey := fmt.Sprintf("%s%v", cacheSysUserIdPrefix, id)
	sysUserLoginNameKey := fmt.Sprintf("%s%v", cacheSysUserLoginNamePrefix, data.LoginName)
	sysUserPhoneKey := fmt.Sprintf("%s%v", cacheSysUserPhonePrefix, data.Phone)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, sysUserEmailKey, sysUserIdKey, sysUserLoginNameKey, sysUserPhoneKey)
	return err
}

func (m *defaultSysUserModel) FindOne(ctx context.Context, id int64) (*SysUser, error) {
	sysUserIdKey := fmt.Sprintf("%s%v", cacheSysUserIdPrefix, id)
	var resp SysUser
	err := m.QueryRowCtx(ctx, &resp, sysUserIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", sysUserRows, m.table)
		return conn.QueryRowCtx(ctx, v, query, id)
	})
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
		return nil, model.ErrNotFound
	default:
		return nil, err
	}
}

func (m *defaultSysUserModel) FindOneByEmail(ctx context.Context, email sql.NullString) (*SysUser, error) {
	sysUserEmailKey := fmt.Sprintf("%s%v", cacheSysUserEmailPrefix, email)
	var resp SysUser
	err := m.QueryRowIndexCtx(ctx, &resp, sysUserEmailKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `email` = ? limit 1", sysUserRows, m.table)
		if err := conn.QueryRowCtx(ctx, &resp, query, email); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
		return nil, model.ErrNotFound
	default:
		return nil, err
	}
}

func (m *defaultSysUserModel) FindOneByLoginName(ctx context.Context, loginName sql.NullString) (*SysUser, error) {
	sysUserLoginNameKey := fmt.Sprintf("%s%v", cacheSysUserLoginNamePrefix, loginName)
	var resp SysUser
	err := m.QueryRowIndexCtx(ctx, &resp, sysUserLoginNameKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `login_name` = ? limit 1", sysUserRows, m.table)
		if err := conn.QueryRowCtx(ctx, &resp, query, loginName); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
		return nil, model.ErrNotFound
	default:
		return nil, err
	}
}

func (m *defaultSysUserModel) FindOneByPhone(ctx context.Context, phone sql.NullString) (*SysUser, error) {
	sysUserPhoneKey := fmt.Sprintf("%s%v", cacheSysUserPhonePrefix, phone)
	var resp SysUser
	err := m.QueryRowIndexCtx(ctx, &resp, sysUserPhoneKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `phone` = ? limit 1", sysUserRows, m.table)
		if err := conn.QueryRowCtx(ctx, &resp, query, phone); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
		return nil, model.ErrNotFound
	default:
		return nil, err
	}
}

func (m *defaultSysUserModel) Insert(ctx context.Context, data *SysUser) (sql.Result, error) {
	sysUserEmailKey := fmt.Sprintf("%s%v", cacheSysUserEmailPrefix, data.Email)
	sysUserIdKey := fmt.Sprintf("%s%v", cacheSysUserIdPrefix, data.Id)
	sysUserLoginNameKey := fmt.Sprintf("%s%v", cacheSysUserLoginNamePrefix, data.LoginName)
	sysUserPhoneKey := fmt.Sprintf("%s%v", cacheSysUserPhonePrefix, data.Phone)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, sysUserRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.LoginName, data.Email, data.Phone, data.Password, data.Salt, data.Nickname, data.Realname, data.Gender, data.Birthday, data.UserType, data.Avatar, data.LoginIp, data.LoginAt, data.Status, data.DeletedAt)
	}, sysUserEmailKey, sysUserIdKey, sysUserLoginNameKey, sysUserPhoneKey)
	return ret, err
}

func (m *defaultSysUserModel) Update(ctx context.Context, newData *SysUser) error {
	data, err := m.FindOne(ctx, newData.Id)
	if err != nil {
		return err
	}

	sysUserEmailKey := fmt.Sprintf("%s%v", cacheSysUserEmailPrefix, data.Email)
	sysUserIdKey := fmt.Sprintf("%s%v", cacheSysUserIdPrefix, data.Id)
	sysUserLoginNameKey := fmt.Sprintf("%s%v", cacheSysUserLoginNamePrefix, data.LoginName)
	sysUserPhoneKey := fmt.Sprintf("%s%v", cacheSysUserPhonePrefix, data.Phone)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, sysUserRowsWithPlaceHolder)
		return conn.ExecCtx(ctx, query, newData.LoginName, newData.Email, newData.Phone, newData.Password, newData.Salt, newData.Nickname, newData.Realname, newData.Gender, newData.Birthday, newData.UserType, newData.Avatar, newData.LoginIp, newData.LoginAt, newData.Status, newData.DeletedAt, newData.Id)
	}, sysUserEmailKey, sysUserIdKey, sysUserLoginNameKey, sysUserPhoneKey)
	return err
}

func (m *defaultSysUserModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheSysUserIdPrefix, primary)
}

func (m *defaultSysUserModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", sysUserRows, m.table)
	return conn.QueryRowCtx(ctx, v, query, primary)
}

func (m *defaultSysUserModel) tableName() string {
	return m.table
}
